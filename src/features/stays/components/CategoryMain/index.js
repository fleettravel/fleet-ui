...
// dates.startDate/endDate are native Date objects (react-day-picker, not
// moment.js — react-dates used to hand back moment objects here, which is
// why this used to be a plain dates.startDate.format("YYYY-MM-DD") call).
...
const CategoryMain = () => {
  ...
  const search = () => {
    ...
    if (dates.startDate) params.set("checkIn", toISODate(dates.startDate));
    if (dates.endDate) params.set("checkOut", toISODate(dates.endDate));
